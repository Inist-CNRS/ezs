...
describe('ISTEXUntar', () => {
    it('should get 10 elements', (done) => {
        const result = [];
        fs.createReadStream('./packages/istex/examples/data/istex-subset-2019-03-15-10.tar.gz')
            .pipe(ezs('ISTEXUntar'))
            // .pipe(ezs('debug'))
            .on('data', (chunk) => {
                result.push(chunk);
    ...
    it('should work on 1000 elements', (done) => {
        const result = [];
        fs.createReadStream('./packages/istex/examples/data/istex-subset-2019-03-15-1000.tar.gz')
            .pipe(ezs('ISTEXUntar'))
            // .pipe(ezs('debug'))
            .on('data', (chunk) => {
                result.push(chunk);
